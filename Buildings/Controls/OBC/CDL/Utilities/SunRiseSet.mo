...
equation
  ...
  annotation (
    ...
    Documentation(
      info="<html>
<p>This block outputs the next sunrise and sunset time. The sunrise time keeps constant until the model time reaches the next sunrise, at which time the output gets updated. Similarly, the output for the next sunset is updated at each sunset. </p>
<p>The time zone parameter is based on UTC time; for instance, Eastern Standard Time is -5h. Note that daylight savings time is not considered in this component. </p>
</html>",
      revisions="<html>
<ul>
<li>
